...
public class ListLocacaoBacking implements Serializable {
	...
	private void buscarDevolvidos() {
		try {
			...
			JSFUtil.addErrorMessage(Titulos.DET_LOCACAO, Mensagem.LOCACAO_ERRO_PESQUISA);
		}
	}
	...
	private boolean validar() {
		if (Valida.isDateNull(dataInicial)) {
			JSFUtil.addErrorMessage(Titulos.DET_LOCACAO, Mensagem.DATA_INICIAL_VAZIO);
			return false;
		}

		if (Valida.isDateNull(dataFinal)) {
			JSFUtil.addErrorMessage(Titulos.DET_LOCACAO, Mensagem.DATA_FINAL_VAZIO);
			return false;
		}

		if (dataInicial.after(dataFinal)) {
			JSFUtil.addErrorMessage(Titulos.DET_LOCACAO, Mensagem.DATA_ERRO);
			return false;
		}
